...
func TR_FcmUpdate(c *gin.Context, db *sql.DB, rds redis.Conn, lang string, reqData map[string]interface{}, resBody map[string]interface{}) int {
	...
	// 토큰정보를 갱신한다
	if len(userkey) == 16 {
		_, err = db.Exec("DELETE FROM FCM_TOKEN WHERE APP_NAME = 'PhotoApp' and USER_KEY = :1", userkey)
		if err != nil {
			global.FLog.Println(err)
			return 9901
		}

		_, err = db.Exec("INSERT INTO FCM_TOKEN (APP_NAME, TOKEN, TYPE, USER_KEY, TOKEN_ERR, UPDATE_TIME) VALUES ('PhotoApp', :1, :2, :3, 'N', sysdate)",
						 reqBody["token"].(string), reqBody["type"].(string), userkey)
		if err != nil {
			if strings.Contains(err.Error(), "ORA-00001") {
				_, err = db.Exec("UPDATE FCM_TOKEN SET TYPE = :1, USER_KEY = :2, TOKEN_ERR = 'N', UPDATE_TIME = sysdate WHERE APP_NAME = 'PhotoApp' and TOKEN = :3",
		...
		if err != nil {
			if strings.Contains(err.Error(), "ORA-00001") {
				_, err = db.Exec("UPDATE FCM_TOKEN SET TYPE = :1, USER_KEY = '', TOKEN_ERR = 'N', UPDATE_TIME = sysdate WHERE APP_NAME = 'PhotoApp' and TOKEN = :2",
