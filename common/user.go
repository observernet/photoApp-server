...
func User_GetLoginInfo(rds redis.Conn, userkey string) (map[string]interface{}, error) {

	// Redis에서 사용자 정보를 가져온다
	rkey := global.Config.Service.Name + ":UserLogin:" + userkey
	rvalue, err := redis.String(rds.Do("GET", rkey))
	if err != nil { return nil, err }

	// Map 데이타로 변환한다
	mapLogin := make(map[string]interface{})
	if err = json.Unmarshal([]byte(rvalue), &mapLogin); err != nil { return nil, err }
