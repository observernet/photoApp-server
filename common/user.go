...
func User_Login(db *sql.DB, rds redis.Conn, userkey string) (string, error) {
	...
	// 사용자 정보를 메모리에 올린다
	if err = User_UpdateInfo(db, rds, userkey); err != nil {
		return "", err
	}

	// 지갑 정보를 메모리에 올린다
	if err = User_UpdateWallet(db, rds, userkey); err != nil {
		return "", err
	}

	// 통계 정보를 메모리에 올린다
	if err = User_UpdateStat(db, rds, userkey); err != nil {
		return "", err
	}
...
func User_UpdateInfo(db *sql.DB, rds redis.Conn, userkey string) error {
...
func User_UpdateWallet(db *sql.DB, rds redis.Conn, userkey string) error {
...
func User_UpdateStat(db *sql.DB, rds redis.Conn, userkey string) error {
	...
	// 당일스냅개수를 가져온다
	err = db.QueryRow("SELECT count(SNAP_IDX) FROM SNAP WHERE SNAP_DATE = " + strToday + " and USER_KEY = '" + userkey + "' and UPLOAD_STATUS = 'V'").Scan(&today_snap_count)
	if err != nil { return err }

	// 당일 라벨개수를 가져온다
	rows, err = db.Query("SELECT IS_ETC, count(LABEL_IDX) FROM SNAP_LABEL WHERE USER_KEY = '" + userkey + "' and TO_CHAR(LABEL_TIME, 'RRRRMMDD') = " + strToday + " GROUP BY IS_ETC")
	if err != nil { return err }
	defer rows.Close()
